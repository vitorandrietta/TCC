...
create table Product(CD_ProductId int identity primary key, NM_ProductName nvarchar(200) not null, DS_ProductDescription nvarchar(200) not null,QT_AvaiableQuantity int not null,CD_Supplier varchar(14) not null)
alter table Product add constraint FK_SupplierPK_ProductFK_1 foreign key(CD_Supplier) references Supplier(CD_CNPJ)
...
alter table Purchase add constraint FK_ClientPK_PurchaseFK_3 foreign key(CD_CLIENTID) references Client(CD_CPF)
 ...
 create table UserLogin(CD_UserId int identity primary key,NM_Username nvarchar(50),CD_Password nvarchar(300),CD_ClientID varchar(11) not null,DT_RegisterDate datetime not null)
 --foreign key com outra tabela, codigo de cliente de outra tabela-> TABELA CLIENT
alter table UserLogin add constraint FK_UserLoginPK_ClientID_ foreign key(CD_ClientID) references Client(CD_CPF)
